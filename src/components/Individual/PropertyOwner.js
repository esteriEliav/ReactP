    ...
    set =(object) => {
     ...
     //this.setState({properties: await postFunction('PropertyOwner/', { id: object.OwnerID})})
       ...
       if (docks && docks[0]) {
        ...
        object.doc = docks.map((dock, index) => <button type='button' key={index} onClick={() => {  fileDownload(dock.docCoding,dock.DocName)}}>{dock.DocName.substring(dock.DocName.lastIndexOf('/'))}</button>)
        }
        return {
